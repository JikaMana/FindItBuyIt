...
const STORES = {
  ...
  "next-ph": {
    store_data: {
      id: "2",
      name: "NEXT CASH and CARRY - Trans Amadi",
      image: "https://images.unsplash.com/photo-1534723452862-4c874018d66d",
      coverImage:
        "https://images.unsplash.com/photo-1441986300917-64674bd600d8",
      rating: 4.5,
      reviews: 128,
      address: "Beside International Breweries(PABOD), Trans Amadi",
      distance: "2.5 km",
      hours: {
        monday: "9:00 AM - 9:00 PM",
        tuesday: "9:00 AM - 9:00 PM",
...
export function StoreDetailsPage() {
  ...
  if (!store) {
    return (
      <h1 className="text-3xl font-medium text-green-600 text-center mt-8">
        Store not found
      </h1>
    );
  }
  ...
  return (
    <div className="bg-gray-50">
      ...
      <div className="relative h-64 md:h-96">
        <img
          src={store.store_data?.coverImage}
          alt={store.store_data?.name}
            ...
            <div className="pb-6 text-white">
              ...
              <div className="flex items-center space-x-4">
                <div className="flex items-center">
                  ...
                  <span className="ml-1">{store.store_data?.rating}</span>
                  <span className="ml-1 text-gray-300">
                    ({store.store_data?.reviews} reviews)
                  </span>
                </div>
                <div className="flex items-center">
                  <MapPin className="w-5 h-5" />
                  <span className="ml-1">{store.store_data?.distance}</span>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="lg:col-span-1 space-y-6">
            ...
            <div className="bg-white rounded-lg shadow-sm p-6 space-y-4">
              <div className="space-y-2">
                <div className="flex items-center text-gray-600">
                  ...
                  <span>{store.store_data?.address}</span>
                </div>
                <div className="flex items-center text-gray-600">
                  <Clock className="w-5 h-5 mr-2" />
                  <div>
                    <div className="font-medium">Today</div>
                    <div>{store.store_data?.hours.monday}</div>
                  </div>
                </div>
                <div className="flex items-center text-gray-600">
                  <Phone className="w-5 h-5 mr-2" />
                  <span>{store.store_data?.phone}</span>
                </div>
              </div>
              <div className="flex space-x-4 pt-4">
                <a
                  href={`https://facebook.com/${store.store_data?.social.facebook}`}
                  target="_blank"
                ...
                <a
                  href={`https://instagram.com/${store.store_data?.social.instagram}`}
                  target="_blank"
                  rel="noopener noreferrer"
                ...
                <a
                  href={`https://twitter.com/${store.store_data?.social.twitter}`}
                  target="_blank"
                  rel="noopener noreferrer"
            ...
            <div className="bg-white rounded-lg shadow-sm p-6">
              ...
              <div className="space-y-2">
                ...
                {store.store_data?.departments.map((dept) => (
                  <button
                    key={dept}
              ...
              <div className="bg-white rounded-lg shadow-sm p-6 space-y-6">
                <div>
                  <h3 className="text-lg font-semibold mb-2">
                    About {store.store_data?.name}
                  </h3>
                  <p className="text-gray-600">
                    {store.store_data?.description}
                  </p>
                </div>
                <div>
                  <h3 className="text-lg font-semibold mb-2">Store Hours</h3>
                  <div className="grid grid-cols-2 gap-4 md:gap-8">
                    {Object.entries(store.store_data?.hours).map(
                      ([day, hours]) => (
                ...
                <div>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    {store.store_data?.amenities.map((amenity) => (
                      <div
                        key={amenity}
